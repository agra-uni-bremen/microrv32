...
class SC_MicroRV32Top(InitInstHexFile:String, InstHexFileSize:Int, InitDataHexFile:String, DataHexFileSize:Int, rv32config:SC_RV32CoreConfig = SC_RV32CoreConfig()) extends Component {
    val io = new Bundle {
    ...
    //cpu core
    val cpu = new SC_SBCore(rv32config)
...
object SC_MicroRV32TopVerilog {
  def main(args: Array[String]) {
    ...
    //   targetDirectory = "rtl"
      ...
      ).generateVerilog(new SC_MicroRV32Top(InsthexFilePath, InstmemSize, DatahexFilePath, DatamemSize, SC_RV32CoreConfig()))
      .printPruned()
  }
}